...
After the contract is deployed, `initializeBlockchainID` must be called in order for the Avalanche blockchain ID the contract is deployed on to be properly set. This initialization step can be performed by anyone, and is required in order to be able to successfully receive messages.
