...
cd contracts
native_erc20_deploy_result=$(forge create --private-key $user_private_key src/Mocks/ExampleERC20.sol:ExampleERC20 --rpc-url $subnet_a_rpc_url)
...
bridge_a_deploy_result=$(forge create --private-key $user_private_key \
    --rpc-url $subnet_a_rpc_url src/CrossChainApplications/ERC20Bridge/ERC20Bridge.sol:ERC20Bridge \
...
bridge_b_deploy_result=$(forge create --private-key $user_private_key \
    --rpc-url $subnet_b_rpc_url src/CrossChainApplications/ERC20Bridge/ERC20Bridge.sol:ERC20Bridge \
    --constructor-args $subnet_b_teleporter_registry_address)
...
bridge_c_deploy_result=$(forge create --private-key $user_private_key \
    --rpc-url $subnet_c_rpc_url src/CrossChainApplications/ERC20Bridge/ERC20Bridge.sol:ERC20Bridge \
    --constructor-args $subnet_c_teleporter_registry_address)
...
result=$(cast call $native_erc20_contract_address "allowance(address,address)(uint256)" $user_address $bridge_a_address --rpc-url $subnet_a_rpc_url)
...
actual_native_chain_id=$(cast call $bridge_token_subnet_b_contract_address "nativeBlockchainID()(bytes32)" --rpc-url $subnet_b_rpc_url)
echo "Bridge token native chain ID: $actual_native_chain_id"
actual_native_bridge=$(cast call $bridge_token_subnet_b_contract_address "nativeBridge()(address)" --rpc-url $subnet_b_rpc_url)
echo "Bridge token native bridge address: $actual_native_bridge"
actual_native_asset=$(cast call $bridge_token_subnet_b_contract_address "nativeAsset()(address)" --rpc-url $subnet_b_rpc_url)
echo "Bridge token native asset: $actual_native_asset"
actual_name=$(cast call $bridge_token_subnet_b_contract_address "name()(string)" --rpc-url $subnet_b_rpc_url)
echo "Bridge token name: $actual_name"
actual_symbol=$(cast call $bridge_token_subnet_b_contract_address "symbol()(string)" --rpc-url $subnet_b_rpc_url)
echo "Bridge token symbol: $actual_symbol"
actual_decimals=$(cast call $bridge_token_subnet_b_contract_address "decimals()(uint8)" --rpc-url $subnet_b_rpc_url)
...
actual_bridge_token_balance=$(cast call $bridge_token_subnet_b_contract_address "balanceOf(address)(uint256)" $user_address --rpc-url $subnet_b_rpc_url)
...
cast send $bridge_token_subnet_b_contract_address "approve(address,uint256)(bool)" $bridge_b_address $approve_amount --private-key $user_private_key --rpc-url $subnet_b_rpc_url
result=$(cast call $bridge_token_subnet_b_contract_address "allowance(address,address)(uint256)" $user_address $bridge_b_address --rpc-url $subnet_b_rpc_url)
if [[ $result -ne $approve_amount ]]; then
    echo $result
...
sleep 10
actual_bridge_token_balance=$(cast call $bridge_token_subnet_c_contract_address "balanceOf(address)(uint256)" $user_address --rpc-url $subnet_c_rpc_url)
...
if [ ! -z "$relayer_address" ]; then
    actual_relayer_redeemable_balance=$(cast call $teleporter_contract_address "checkRelayerRewardAmount(address,address)(uint256)" $relayer_address $native_erc20_contract_address --rpc-url $subnet_a_rpc_url)
...
cast send $bridge_token_subnet_c_contract_address "approve(address,uint256)(bool)" $bridge_c_address $approve_amount --private-key $user_private_key --rpc-url $subnet_c_rpc_url
result=$(cast call $bridge_token_subnet_c_contract_address "allowance(address,address)(uint256)" $user_address $bridge_c_address --rpc-url $subnet_c_rpc_url)
if [[ $result -ne $approve_amount ]]; then
    echo $result
...
cast send $bridge_c_address "bridgeTokens(bytes32,address,address,address,uint256,uint256,uint256)" \
    ...
    $primary_fee_amount \
    $secondary_fee_amount \
    --private-key $user_private_key --rpc-url $subnet_c_rpc_url
echo "Sent a transaction to unwrap tokens from Subnet C back to Subnet A."
sleep 10

# Check the balance of the native token after the unwrap
actual_native_token_default_account_balance=$(cast call $native_erc20_contract_address "balanceOf(address)(uint256)" $user_address --rpc-url $subnet_a_rpc_url)
