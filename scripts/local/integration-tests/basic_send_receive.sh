...
echo "Sending call to teleporter contract address $teleporter_contract_address $subnet_a_chain_id_hex $subnet_b_rpc_url"
result=$(cast call $teleporter_contract_address "getNextMessageID(bytes32)(uint256)" $subnet_a_chain_id_hex --rpc-url $subnet_b_rpc_url)
...
retry_count=0
received=$(cast call $teleporter_contract_address "messageReceived(bytes32,uint256)(bool)" $subnet_b_chain_id_hex $startID --rpc-url $subnet_a_rpc_url)
until [[ $received == "true" ]]
do
    if [[ retry_count -ge 10 ]]; then
