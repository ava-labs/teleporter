...
cast send $subnet_a_native_erc20_contract "mint(uint256)" 500000000000000000 --private-key $user_private_key --rpc-url $subnet_a_url

# Check the user's current ERC20 token balances on each chain before bridging.
native_erc20_balance=$(cast call $subnet_a_native_erc20_contract "balanceOf(address)(uint256)" $user_address --rpc-url $subnet_a_url)
subnet_b_bridged_token_balance=$(cast call $subnet_b_bridge_token_contract "balanceOf(address)(uint256)" $user_address --rpc-url $subnet_b_url)
subnet_c_bridged_token_balance=$(cast call $subnet_c_bridge_token_contract "balanceOf(address)(uint256)" $user_address --rpc-url $subnet_c_url)
