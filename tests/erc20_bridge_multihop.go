...
func ERC20BridgeMultihop(network network.Network) {
	...
	Expect(err).Should(BeNil())
	subnetCTeleporterMessenger, err := teleportermessenger.NewTeleporterMessenger(teleporterContractAddress, subnetCInfo.ChainRPCClient)
	Expect(err).Should(BeNil())
	...
	receipt, messageID := submitCreateBridgeToken(ctx, subnetAInfo, subnetBInfo.BlockchainID, erc20BridgeAddressB, nativeERC20Address, nativeERC20Address, createBridgeTokenMessageFeeAmount, fundedAddress, fundedKey, erc20BridgeA, subnetATeleporterMessenger)
	...
	bridgeTokenSubnetBAddress, err := erc20BridgeB.NativeToWrappedTokens(&bind.CallOpts{}, subnetAInfo.BlockchainID, erc20BridgeAddressA, nativeERC20Address)
	Expect(err).Should(BeNil())
	...
	receipt, messageID = submitCreateBridgeToken(ctx, subnetAInfo, subnetCInfo.BlockchainID, erc20BridgeAddressC, nativeERC20Address, nativeERC20Address, createBridgeTokenMessageFeeAmount, fundedAddress, fundedKey, erc20BridgeA, subnetATeleporterMessenger)
	// Relay message
	...
	bridgeTokenSubnetCAddress, err := erc20BridgeC.NativeToWrappedTokens(&bind.CallOpts{}, subnetAInfo.BlockchainID, erc20BridgeAddressA, nativeERC20Address)
	Expect(err).Should(BeNil())
	...
	receipt, messageID = bridgeToken(ctx, subnetAInfo, subnetBInfo.BlockchainID, erc20BridgeAddressB, nativeERC20Address, fundedAddress, totalAmount, primaryFeeAmount, secondaryFeeAmount, fundedAddress, fundedKey, erc20BridgeA, true, subnetAInfo.BlockchainID, subnetATeleporterMessenger)
	...
	approveBridgeToken(ctx, subnetBInfo, bridgeTokenSubnetBAddress, bridgeTokenB, amount, erc20BridgeAddressB, fundedAddress, fundedKey)
	...
	receipt, messageID = bridgeToken(ctx, subnetBInfo, subnetCInfo.BlockchainID, erc20BridgeAddressC, bridgeTokenSubnetBAddress, fundedAddress, totalAmount, primaryFeeAmount, secondaryFeeAmount, fundedAddress, fundedKey, erc20BridgeB, false, subnetAInfo.BlockchainID, subnetBTeleporterMessenger)
	...
	actualRelayerRedeemableBalance, err := subnetATeleporterMessenger.CheckRelayerRewardAmount(&bind.CallOpts{}, fundedAddress, nativeERC20Address)
	Expect(err).Should(BeNil())
	...
	approveBridgeToken(ctx, subnetCInfo, bridgeTokenSubnetCAddress, bridgeTokenC, expectedAmount, erc20BridgeAddressC, fundedAddress, fundedKey)
	...
	primaryFeeAmount = big.NewInt(0).Mul(big.NewInt(1e18), big.NewInt(1))
	secondaryFeeAmount = big.NewInt(0)
	receipt, messageID = bridgeToken(ctx, subnetCInfo, subnetAInfo.BlockchainID, erc20BridgeAddressA, bridgeTokenSubnetCAddress, fundedAddress, totalAmount, primaryFeeAmount, secondaryFeeAmount, fundedAddress, fundedKey, erc20BridgeC, false, subnetAInfo.BlockchainID, subnetCTeleporterMessenger)
...
func submitCreateBridgeToken(
	...
	tx, err := transctor.SubmitCreateBridgeToken(opts, destinationChainID, destinationBridgeAddress, nativeToken, messageFeeAsset, messageFeeAmount)
	Expect(err).Should(BeNil())
...
func bridgeToken(
	...
	tx, err := transctor.BridgeTokens(opts, destinationChainID, destinationBridgeAddress, nativeToken, recipient, totalAmount, primaryFeeAmount, secondaryFeeAmount)
	Expect(err).Should(BeNil())
