...
//
// Solidity: function registeredDestinations(bytes32 destinationBlockchainID, address destinationBridgeAddress) view returns(bool registered, uint256 collateralNeeded, uint256 tokenMultiplier, bool multiplyOnSpoke)
...
// TeleporterTokenSourceDestinationRegisteredIterator is returned from FilterDestinationRegistered and is used to iterate over the raw logs and unpacked data for SpokeRegistered events raised by the TeleporterTokenSource contract.
...
// TeleporterTokenSourceDestinationRegistered represents a SpokeRegistered event raised by the TeleporterTokenSource contract.
...
//
// Solidity: event SpokeRegistered(bytes32 indexed destinationBlockchainID, address indexed destinationBridgeAddress, uint256 initialCollateralNeeded, uint256 tokenMultiplier, bool multiplyOnSpoke)
func (_TeleporterTokenSource *TeleporterTokenSourceFilterer) FilterDestinationRegistered(opts *bind.FilterOpts, destinationBlockchainID [][32]byte, destinationBridgeAddress []common.Address) (*TeleporterTokenSourceDestinationRegisteredIterator, error) {
	...
	logs, sub, err := _TeleporterTokenSource.contract.FilterLogs(opts, "SpokeRegistered", destinationBlockchainIDRule, destinationBridgeAddressRule)
	if err != nil {
		return nil, err
	}
	return &TeleporterTokenSourceDestinationRegisteredIterator{contract: _TeleporterTokenSource.contract, event: "SpokeRegistered", logs: logs, sub: sub}, nil
}
...
func (_TeleporterTokenSource *TeleporterTokenSourceFilterer) WatchDestinationRegistered(opts *bind.WatchOpts, sink chan<- *TeleporterTokenSourceDestinationRegistered, destinationBlockchainID [][32]byte, destinationBridgeAddress []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _TeleporterTokenSource.contract.WatchLogs(opts, "SpokeRegistered", destinationBlockchainIDRule, destinationBridgeAddressRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				...
				if err := _TeleporterTokenSource.contract.UnpackLog(event, "SpokeRegistered", log); err != nil {
					return err
				}
				event.Raw = log
